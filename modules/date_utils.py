...
def extract_date(text: str, base_dt: datetime = None) -> Tuple[Optional[datetime], bool]:
    ...
    days_after_date = re.search(r'(\d+)\s*days?\s*(?:after|from)\s+(\d{1,2})(?:st|nd|rd|th)?\s*(?:of\s+)?(' + month_pattern + ')', text_lower)
    if days_after_date:
    ...
    days_after_date_month_first = re.search(r'(\d+)\s*days?\s*(?:after|from)\s+(' + month_pattern + ')\s+(\d{1,2})(?:st|nd|rd|th)?', text_lower)
    ...
    # When no month is specified and day has passed in current month, assume next month
    bare_day_pattern = r'\b(\d{1,2})(?:st|nd|rd|th)?\b'
    ...
    for bare_day_match in bare_day_matches:
        ...
        # Pattern: "from X to Y" or "X-Yam" or "X-Y pm" or "X am/pm"
        time_range_patterns = [
